...
namespace DAL
{
    public class Comment
    {
        ...
        public IEnumerable<Model.Comment> GetWorkPage(Model.CommentPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by commentTime desc) as num, Comment.* from Comment where Comment.workId=@workId)  join workinfo on Comment.workId=workinfo.workId)";
        ...
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment (commentID,workId,userName,CommentContent,CommentTime)" +
                    "valuec(@commentID,@workId,@userName,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENDTIIY";
